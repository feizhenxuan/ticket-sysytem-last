...
public class IntentDetector {
    ...
    private static final Pattern MOVIE_PATTERN = Pattern.compile(
            "(什么电影|有什么电影|电影推荐|推荐.*电影|评分.*高|最近.*上映|热映|正在.*映|好看.*电影)"
    );
    ...
    public Intent detect(String text) {
        ...
        // 简单打招呼不算 reject，让 LLM 处理
        if (lower.length() <= 4 && (lower.contains("你好") || lower.contains("hi")
